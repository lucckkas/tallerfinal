...
> La entrada de datos que tiene que recibir el Front End debe ser compatible con los datos originales del conjunto MHealth. Para consumir la API, es necesario aplicar todo el procesamiento previo a los datos para que sea compatible con el modelo predictivo. Por lo tanto, es necesario generalizar el trabajo realizado en la etapa de pre-procesamiento de los datos en funciones para procesar nuevos casos a modo de Pipeline.
...
-   Lenguaje y framework de libre elección (por ejemplo: Python + FastAPI/Flask/Django, Node.js/Express, Java/Spring, etc.).
-   Debe existir una API claramente definida con, al menos:
    ...
    -   **POST /detect** (nombre a elección razonable):
        -   Entrada: archivo `.log` (por ejemplo, JSON con las características necesarias). Predecir los datos de un sujeto que no participó en el entrenamiento.
        -   Salida: resultado de detección de anomalías (Categoría, y corresponde la actividad que está realizando el sujeto), más cualquier información adicional útil.
...
-   Requisitos mínimos:
    ...
    -   Debe permitir:
        ...
        -   Mostrar de forma clara el resultado de la detección (mensaje, gráficos simples, etc.).
...
-   **Duración:** 8 minutos de presentación + 2 minutos de preguntas.
-   La presentación debe incluir:
    ...
    -   Demostración en vivo del sistema (live demo): mostrar el sistema funcionando, desde la interacción del usuario hasta la respuesta.
    -   Breve reflexión sobre dificultades técnicas y aprendizajes.

> La presentación (con demo en vivo) es requisito para aprobar el proyecto final. Si el estudiante no realiza la presentación, no aprueba el proyecto, independientemente de la nota del informe o del código. Eventualmente habrá un plazo para una presentación recuperativa.
